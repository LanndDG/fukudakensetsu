...
//----------------------------------------------------------------------
//　改変：デザイン・ファクトリー・ジャパン合同会社　2020.05.07
//　（確認画面の外部テンプレート化、管理者宛メッセージの変数化、エラーメッセージ出力の関数化、完了画面なしの場合にTOPに転送　等）
//----------------------------------------------------------------------
...
//　確認画面項目出力キー　##CONFRIM_TABLE##　を入れたテンプレートファイル　confirm.tpl.html　を用意してください。
// ※エラー出力兼用のため、確認画面を表示しない場合でもconfirm.tpl.htmlは必要です。
$confirmDsp = 0;
...
$thanksPage = "http://webcircle09.heteml.net/demo/dg/xxxxxxx";
...
この度は、お問い合わせをいただき、誠にありがとうございました。お送りいただきました内容を確認の上、折り返しご連絡させていただきます。
なお、3営業日を経過しても連絡がない場合は、送信トラブル等の可能性もありますので、大変お手数ではございますが、もう一度フォームよりお問い合わせいただくか、お電話にてお問い合わせをお願いいたします。
...
福田建設株式会社
本社： 埼玉県八潮市西袋423番地
【TEL】048-928-3536(代表) 【FAX】048-927-5416
──────────────────────
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Email && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
				$errm .= "<li>【".$key."】はメールアドレスの形式が正しくありません。</li>\n";
				$empty_flag = 1;
			}
		}
	}
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	//管理者宛に届くメールをセット
	$adminBody = mailToAdmin($_POST,$subjectt,$adminMailText,$mailFooterDsp,$mailSignature,$encode,$confirmDsp);
...
function confirmOutput($arr){
	...
	foreach($arr as $key => $val) {
		...
		$html .= "<tr><th>".$key."</th><td><span>".$out;
		$html .= '</span><input type="hidden" name="'.$key.'" value="'.str_replace(array("<br />","<br>"),"",$out).'" />';
		$html .= "</td></tr>\n";
	}
	//トークンをセット
	if($useToken == 1 && $confirmDsp == 1){
...
//管理者宛送信メールボディ
function mailToAdmin($arr,$subject,$adminMailText,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	$adminBody = '';
	$adminBody.=$adminMailText;
	$adminBody.="＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n\n";
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<li>必須項目【".h($key)."】が未入力です。</li>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
		...
		if($existsFalg != 1){
			$res['errm'] .= "<li>必須項目【".$requireVal."】が未選択です。</li>\n";
			$res['empty_flag'] = 1;
		}
	}
	return $res;
...
function refererCheck($Referer_check,$Referer_check_domain){
	if($Referer_check == 1 && !empty($Referer_check_domain)){
		if(strpos($_SERVER['HTTP_REFERER'],$Referer_check_domain) === false){
			return copyright().exit('<br /><br /><p align="center">リファラチェックエラー。フォームページのドメインとこのファイルのドメインが一致しません</p>');
		}
	}
}
function copyright(){
...
//テンプレート出力
function htmlTemplate($outputPos) {
	global $template;
	$template = [];
	$table_key = "##CONFIRM_TABLE##";
	$template_html = file_get_contents("confirm.tpl.html");
	$pos = mb_strpos( $template_html, $table_key, 0, "utf-8"  );
	if($pos) {
		$template["header"] = mb_substr( $template_html, 0, $pos, "utf-8" );
		$template["footer"] = mb_substr( $template_html, $pos + strlen( $table_key ), mb_strlen( $template_html, "utf-8" ), "utf-8" );
	}
	print $template[$outputPos];
}
...
function errorMes() {
	global $errm;
	echo <<< ERTXT
	<h3 class="conf_title">入力エラー</h3>
	<p>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</p>
	<ul class="error_messe">
		$errm
	</ul>
	<div class="submit">
